...
I am a Real estate investing data analyst teaching myself to build. Interested in forecasting, experimentation, and data products that reach real users.
...
`PostgreSQL` `Python` `pandas` `scikit-learn` `SHAP` `Optuna` `TimescaleDB` `Snowflake` `Next.js` `TypeScript` `Mapbox GL` `deck.gl` `Vercel` `Tableau`
